...
C     19.  Passed OTEFF as arguments to account for O2 transfer efficiency in model (12-10-13, KAB)
C
C	April 14, 2009
C
C	VARIABLES
...
C
     ...
     +YO2,YN2,Z,AA,BB,CC,BNOT,LNOT,ELEV,DT,TE(100),XLOC,DEPTH,
     +CO2M(100),LOCAT(100),DOAMB,COMGP,CNMGP,WSEL,PATM,SAL(100),
     +DIFFEL,GROSSMT,FGONOT,DNAMB,DMPR,TAVG,SUMTEMP,SUMSAL,
     +LAMBNOT,FRCONOT,RBNOT,H,DYDX(8),Y(8),YOUT(8),
     +DENSE20,OTEFF,FRNOT,VDIFF,FR,BUOY,DCO2,QGFRAC,LDIFF,TDS(100),
     +JDAY,EL(70),DELTAC,COMGNOT,ELEVT,QWT,TPLUMET,COMGPT,QWD(100),
     +PWD(100),BWD(100),AWD(100),
...
C     Assume that gas bubbles are composed of oxygen and nitrogen only.
      ...
      FRACN=1.0-FRACO
      DEPTH=WSEL-DIFFEL
      Z=DEPTH !from free surface to diffusers, commented by Chris
      ELEV=DIFFEL
      X=0.
C
...
C
C     CONSTANTS
      ALPHA=0.083 ! Schladow 1993 original 0.11
      G=9.80665
      GAMMA=6.9E-4
      IF(LAKE.EQ.1)THEN
         LAMBDA=0.93
...
C
      ...
      QWT=QW
      TPLUMET=TPLUME
      COMGPT=COMGP !*0.9 ! Added '*0.9' on 26 Nov to try to decrease O2 transf.
      LAYTOP=LAYDIFF-JJ
      BTOP=B
      RETURN
      END
