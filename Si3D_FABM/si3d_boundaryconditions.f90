...
!------------------------------------------------------------------------
   ...
   DO nn = 1, nopen
      ...
      ! Allocate space for the array of data first time in loop - Mod by Chris rdtr+2 back to ntr (ntr+2 02/08/17)
      IF ( nn == 1) THEN
      ...
      WRITE (UNIT=openbcfmt, FMT='("(10X,",I3,"G11.2)")') rdtr+2
 
      ! Read data array
      varsOpenBC = 0;
      DO j = 1, nptsOpenBC
         READ (UNIT=i52, FMT=openbcfmt, IOSTAT=ios) &
              (varsOpenBC(nn,i,j), i=1,rdtr+2)
...
!------------------------------------------------------------------------
         ...
         CASE(1)
           ...
           DO j = js, je;
             ...
             DO k = k1, kmz(i,j)
               ...
               huWB(k,j)= h(k,l)
               !Q1 = Q1 + uhWB(k,j)*dy
               !TotalFLOW = TotalFLOW + uhWB(k,j)*dy ! positive means inflow
             ENDDO 
           ENDDO 
         CASE(3)
           i   = isbc(nn); 
           ...
           DO j = js, je; 
             ...
             DO k = k1, kmz(i,j)
               ...
               huEB(k,j) = h(k,l)
               !Q3 = Q3 - uhEB(k,j)*dy
               !TotalFLOW = TotalFLOW - uhEB(k,j)*dy ! positive means outflow
             ENDDO
           ENDDO
         CASE(2)
           j   = jsbc(nn); 
           ...
           DO i = is, ie; 
             ...
             DO k = k1, kmz(i,j)
               ...
               !hvNB(k,j) = h(k,l)
               hvNB(k,i) = h(k,l)
               !Q2 = Q2 - vhNB(k,i)*dx ! positive means outflow
               !TotalFLOW = TotalFLOW - vhNB(k,i)*dx ! positive means outflow
             ENDDO
           ENDDO
         CASE(4)
           j  = jsbc(nn); 
           ...
           DO i = is, ie; 
             ...
             DO k = k1, kmz(i,j)
               ...
               !hvSB(k,j) = h(k,l)
               hvSB(k,i) = h(k,l)
               !Q4 = Q4 + vhSB(k,i)*dx ! positive means outflow
               !TotalFLOW = TotalFLOW + vhSB(k,i)*dx ! positive means inflow
             ENDDO
           ENDDO
         END SELECT   
   ...
   !IF(n == 1) THEN 
   !    OPEN(unit=134,file='flowNG.txt',FORM='FORMATTED',IOSTAT=ios)
   !ENDIF
   !IF ( istep == 2 ) THEn
   !   WRITE (UNIT=134, FMT='(6E20.11)') thrs, TotalFLOW, Q1, Q2, Q3, Q4 
   !ENDIF
...
!------------------------------------------------------------------------
   ...
   ! Save and read new frames if thrs > thrsNGB .......
       ...
       CASE (1)
         ...
         DO j = js, je
           ...
           DO k = k1x,kmx
            ...
            !.....Final explicit term.....
            ex(k,l) = uhpp(k,l) - twodt1*(advx*iadv) !-g*xslope*hup(k,l))
       ...
       CASE (2)
         ...
         DO i = is, ie
           ...
           DO k = k1y,kmy
            ...
            !.....Final explicit term.....
            ex(k,l) = vhpp(k,l) - twodt1*(advy*iadv-(g*yslope*hvp(k,l))) !FJRGOTM
	 ...
	 ! to the North of a water column with EW flow specified
         IF (mask2d(i,j+1)) THEN 
           ...
           DO k = ks,ke
             ...
             !.....Final explicit term.....
             ex(k,l) = vhpp(k,l) - twodt1*(advy*iadv-(g*yslope*hvp(k,l)))
...
! 
!  FJR - Modify to account for cells which are along two boundaries (
!        (i.e. along a corner)
!
!------------------------------------------------------------------------
...
END SUBROUTINE MODvel4openBC_OLD
    ...
    DO nn = 1, nopen
        ...
        SELECT CASE ( itype(nn) )
            ...
            CASE (1)
                ...
                END SELECT ! changed by Chris from ENDSELECT

            ! ... Case 2 -- surface flow boundary conditions to account for inflow substances (modified by Chris) DISABLED
            CASE (-999)

                IF (idbg == 1) PRINT *, " entering openbcTracer assignment"
                !.....Get new boundary value of scalar.....
                ! ... Set value of scalar at OpenBC nn -
                dthrs_sal = dtsecOpenBC/3600.
                salbc = parab(0.,thrs,varsOpenBC(nn,2+itr,:),dthrs_sal)
                ...
                SELECT CASE ( iside(nn) )
                    ...
                    CASE (1)
                        ...
                        DO j = js, je
                            ...
                            kms = kmz(i,j)
                            k1s = k1z(i,j)
                            ! Compute tracer at the domain entrance
                            DO k = k1s,kms
                                tracer(k,l,id_init_var(itr)) = salbc
                            ENDDO
                        ENDDO
...
!------------------------------------------------------------------------
   ...
   CASE (10) ! Space & Time varying met. variables - Heat budget on run-time (I) mode
     ...
     CALL InitializeInterpolationMethods ! Barnes SaltonSea
     !CALL InitializeBarnesInterpolation ! Barnes Correcte
   ...
   CASE (11) ! Space & Time varying met. variables - Heat budget on run-time (II) mode
     ...
     ! ... Initialize Interpolation Schemes (weights for Barnes)
     CALL InitializeInterpolationMethods ! Barnes SaltonSea
     !CALL InitializeBarnesInterpolation ! Barnes Corrected
...
!------------------------------------------------------------------------
  ...
  ! ... Output title and prepare to print
  IF (MOD(n,MAX(500,1))==0 .OR. n == 1) THEN ! update to screen every 500 steps
      PRINT *, '****************************************************'
      PRINT *, '-------OUTPUT FROM DisQsw ROUTINES---------------'
      PRINT *, '****************************************************'
  ENDIF
  ...
  DO l = 1, lm
       ...
       CASE (2:)
         ...
         DO k = k1s+1, kms+1          
             SELECT CASE (radischm)
                 CASE (1)
                     QswFr(k-1,l) = SolarFr_H(zfromt(k-1,l)) - SolarFr_H(zfromt(k,l))
                 CASE (2:)
                     QswFr(k-1,l) = SolarFr_K(zfromt(k-1,l)) - SolarFr_K(zfromt(k,l))
             END SELECT
         END DO
         ...
         ! ... Output solar absorption for the first step for depth > 8m
!         IF ((n == 1) .AND. zfromt(kms+1) > 8) THEN
!             PRINT *, 'loc remFr&Fraction', i, j, remFr, QswFr(k-6,l)-remFr / nwlayers
!             PRINT *, '****************************************************'
!         ENDIF
...
!************************************************************************
REAL FUNCTION SolarFr_K ( depth )
!************************************************************************
!
!  Purpose: To calculate the attenuation of solar irradiance penetrating  
!           the water column through the free surface. It uses the model
!           proposed in Kara et al 2005 eq. (2) (3)
!
!           Included by Chris 03/12/2014
!
!------------------------------------------------------------------------
  ...
  ! ... Local variables
  REAL            :: z, irr_gamma
  REAL, PARAMETER :: zA = 0.5

  ! red spectrum penetration scale is 2m (e-folding depth of 0.5m)
  irr_gamma=max(0.27,(0.695-5.7*eta))
  z = depth

  SolarFr_K = (1-irr_gamma)*exp(-z/zA)+irr_gamma*exp(-z*eta);


END FUNCTION SolarFr_K

!************************************************************************
REAL FUNCTION SolarFr_H ( depth )
!************************************************************************
!
!  Purpose: To calculate the attenuation of solar irradiance penetrating
...
!------------------------------------------------------------------------
  ...
  IF ( eta .GE. 0.1) THEN
    ...
    IF ( z<zA ) THEN
      SolarFr_H = (1.-BetaSol*z/zA)
    ELSE
      SolarFr_H = (1.-BetaSol)*EXP(-eta*(z-zA));
    ENDIF
  ELSE
    SolarFr_H = EXP(-eta*z)
  ENDIF

END FUNCTION SolarFr_H
  ...
  CASE (11) ! Interpolation
    ...
    DO l = 1, lm;
      ...
      DO imet = 1, nmetstat
        ...
        Ta   = Ta     + WeightSt(i,j,imet) * Ta2D  (imet)
        RH   = RH     + WeightSt(i,j,imet) * RH2D  (imet)
        Qlw  = Qlw    + WeightSt(i,j,imet) * Qlw2D (imet)
        uair(i,j) = uair(i,j) + WeightSt(i,j,imet) * uair2D(imet)
        vair(i,j) = vair(i,j) + WeightSt(i,j,imet) * vair2D(imet)
      ENDDO 
...
!************************************************************************
 SUBROUTINE DistributeMomentumHeatSourcesOLD
!************************************************************************
!
!  Purpose: To construct a 2D met field from discrete variables & 
...
END SUBROUTINE DistributeMomentumHeatSourcesOLD
...
!***********************************************************************
FUNCTION nearestn ( frstpt, x, fx, dx )
!***********************************************************************
!
!  Purpose: To select the nearest functional value
!           within the array fx.
!
!-----------------------------------------------------------------------
   ...
   ind=FLOOR((x/dx)+0.5)+1
   nearestn=fx(ind)
   !Print *, nearestn, ind, x, dx, fx(ind)

END FUNCTION nearestn
