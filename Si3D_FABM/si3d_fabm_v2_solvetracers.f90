...
!-------------------------------------------------------------------------
   ...
   CHARACTER(LEN=256), ALLOCATABLE, DIMENSION(:) :: get_fabm_output_vars
...
!------------------------------------------------------------------------
    ...
    CHARACTER(LEN=64) :: var_input_file = "si3d_fabm_vars.txt"
    INTEGER :: istat
    ...
    ALLOCATE(get_fabm_output_vars(ntr),STAT=istat)
    IF (istat /= 0) CALL allocate_error ( istat, 351 )

    CALL ReadVarList(var_input_file,ntr,get_fabm_output_vars)
...
!------------------------------------------------------------------------
    ...
    ALLOCATE (id_FABMtr_out(nFABMtr_out), id_tracer_out(nFABMtr_out), STAT=istat)
    ...
    DO ivar = 1, nFABMtr_out
        IF (id_tracer_out(ivar) == 0) THEN
            PRINT *, "Tracer ID unassigned: ", ivar
            STOP
        ENDIF
    ENDDO
